...
export default function ProfileScreen() {
  ...
  const user = {
    ...
    profile_picture: userState?.user?.data?.profile_pic,
  };
      ...
      <ScrollView showsVerticalScrollIndicator={false} className="flex-1">
        ...
        <View className="mt-10 px-6 pb-10">
          ...
          <View className="bg-white rounded-[32px] overflow-hidden border border-slate-100 shadow-sm">
            <MenuOption icon="person-outline" label="Personal Information" />
            <MenuOption icon="card-outline" label="Payment Methods" />
            <MenuOption icon="time-outline" label="Rental History" />
            <MenuOption
          ...
          {/* Logout Button */}
          <TouchableOpacity
            className="mt-8 bg-red-50 py-5 rounded-[24px] items-center border border-red-100"
            onPress={Logout}
          >
